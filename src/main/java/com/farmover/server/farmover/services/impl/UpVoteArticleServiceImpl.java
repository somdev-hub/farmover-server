...
public class UpVoteArticleServiceImpl implements UpVoteArticleService{
    ...
    public void upVote(UpVoteArticleRequest request) {
        ...
        if(ls.size()==0){
            ...
            aUpVoteArticle.setUname(request.getUname());
    ...
    @Override
    public void deleteUpVote(String uname, int articleId) {
        ArticleDetail  articleDetail = articleRepo.findById(articleId).orElseThrow(() -> new RuntimeException("Video not found"));
        List<UpVoteArticle> ls = uRepo.findByArticleAndUname(articleDetail, uname);
        if(ls.size()!=0){
            articleDetail.setUpCount(articleDetail.getUpCount()-1);
